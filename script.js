...
const leftCalendarSelect = document.querySelector(".left");
...
setTimeout(() => {
  if (leftCalendarSelect) {
    leftCalendarSelect.addEventListener("click", (event) => {
      if (event.target.className && event.target.className.includes("day")) {
        const currYear = document.querySelector(".left .year-num");
        const currMonth = document.querySelector(".left .month-num");
        if (currYear && currMonth) {
          if (
            sampleDataList[currYear.innerText] &&
            sampleDataList[currYear.innerText][currMonth.innerText] &&
            sampleDataList[currYear.innerText][currMonth.innerText][
              event.target.innerText
            ]
          ) {
            event.target.style.color = "blue";
            event.target.setAttribute("id", "color_remove");
            // alert(
            //   sampleDataList[currYear.innerText][currMonth.innerText][
            //     event.target.innerText
            //   ]
...
window.onclick = function (event) {
  if (event.target == modalSelector) {
    ...
    document.getElementById("color_remove").style.color = "black";
    document.getElementById("color_remove").removeAttribute("color_remove");
  }
};
